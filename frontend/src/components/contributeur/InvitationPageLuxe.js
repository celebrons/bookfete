// C:\Users\USER\bookfete\frontend\src\components\contributeur\InvitationPageLuxe.js
...
const InvitationPageLuxe = () => {
  ...
  useEffect(() => {
    ...
    const loadInvitationAndDraft = async () => {
      try {
        ...
        const savedDraft = localStorage.getItem(`draft_invite_${token}`);
  ...
  const handleAutoSave = () => {
    try {
      localStorage.setItem(`draft_invite_${token}`, JSON.stringify({
        name: formData.name,
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      localStorage.removeItem(`draft_invite_${token}`);
      setSubmitted(true);
      ...
      <div className="invitation-card">
        ...
        <div className="card" style={{ 
          ...
          <p className="body-text" style={{ textAlign: 'center', margin: 0 }}>
            ...
            </span> vous a invité à contribuer au chapitre{' '}
            <span style={{ fontWeight: '600', color: 'var(--gold)' }}>
          ...
          <div className="card" style={{ 
            ...
            <div>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-light)' }}>
                Passé cette date, vos modifications ne seront plus possibles
              </p>
            </div>
          </div>
        )}
        ...
        {questions.length > 0 && (
          <div className="card" style={{ marginBottom: 'var(--space-xl)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-xs)', marginBottom: 'var(--space-md)' }}>
              ...
              <span className="label-gold">Pour vous aider</span>
            </div>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              {questions.map((q, idx) => (
                <li key={idx} style={{
                  ...
                  borderBottom: idx < questions.length - 1 ? 'var(--border-fine)' : 'none'
                }}>
                  <span style={{
                    width: '24px',
                    height: '24px',
                  ...
                  <span className="body-text">{q}</span>
                </li>
              ))}
            </ul>
          </div>
...
export default InvitationPageLuxe;
